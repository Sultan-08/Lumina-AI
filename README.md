# 🎓 Lumina: AI Study Companion
Transforming Notes into Knowledge with Lumina.

Lumina is a high-performance educational prototype that turns static study materials into interactive learning experiences. By leveraging Gemini 1.5 Pro, it automates the most tedious parts of studying: summarizing, creating flashcards, and generating active recall quizzes.

# 🚀 Key Features
📄 Intelligent Syllabus Processor: Upload text files for instant structured analysis.

💡 Adaptive Explanation Engine: Toggle between "Standard Academic" and "ELI10" (Explain Like I'm 10) modes for varying levels of complexity.

🧠 Visual Mind Mapping: Automatically generates hierarchical text-based mind maps to visualize connections between concepts.

⚡ Active Recall Suite:

Auto-Flashcards: Ready-to-use pairs for Anki/Quizlet.

Instant Quizzes: Dynamic MCQs based strictly on your uploaded content.

📝 Last-Minute Revision: Generates "Cheat Sheets" with high-priority formulas and "Big Idea" summaries.

## Run Locally

**Prerequisites:**  Node.js

1. Install dependencies:
   `npm install`
2. Set the `GEMINI_API_KEY` in [.env.local](.env.local) to your Gemini API key
3. Run the app:
   `npm run dev`
